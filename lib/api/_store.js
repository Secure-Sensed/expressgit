...
async function listShipments() {
  if (supabase) {
    try {
      ...
      // Fall back to the in-memory store if Supabase is unavailable or misconfigured.
      console.error("Supabase listShipments failed, falling back to in-memory store:", error);
    }
  }
...
async function trackShipments(mode, queries = []) {
  ...
  try {
    ...
    console.error("Supabase trackShipments failed, falling back to in-memory store:", error);
  }

  // Fallback: use in-memory store for tracking.
  return cleanedQueries.map((query) => {
    const shipment = findShipmentInMemory(lookupMode, query);
    if (!shipment) {
      return { query, found: false, shipment: null };
    }
    return { query, found: true, shipment: clone(shipment) };
  });
}
...
async function upsertShipment(input) {
  ...
  if (supabase) {
    try {
      const { data: existingRows, error: existingError } = await supabase
        .from("shipments")
        .select(
          "id,reference_number,tcn,proof_of_delivery,current_lat,current_lng,origin_lat,origin_lng,destination_lat,destination_lng,customer_email,customer_name"
        )
      ...
      const payload = {
        ...
        current_lat: Object.prototype.hasOwnProperty.call(shipment, "currentLat")
          ? shipment.currentLat
          : ((existing && existing.current_lat) || null),
        current_lng: Object.prototype.hasOwnProperty.call(shipment, "currentLng")
          ? shipment.currentLng
          : ((existing && existing.current_lng) || null),
        origin_lat: Object.prototype.hasOwnProperty.call(shipment, "originLat")
          ? shipment.originLat
          : ((existing && existing.origin_lat) || null),
        origin_lng: Object.prototype.hasOwnProperty.call(shipment, "originLng")
          ? shipment.originLng
          : ((existing && existing.origin_lng) || null),
        destination_lat: Object.prototype.hasOwnProperty.call(shipment, "destinationLat")
          ? shipment.destinationLat
          : ((existing && existing.destination_lat) || null),
        destination_lng: Object.prototype.hasOwnProperty.call(shipment, "destinationLng")
          ? shipment.destinationLng
          : ((existing && existing.destination_lng) || null),
        customer_email: shipment.customerEmail || (existing && existing.customer_email) || null,
        customer_name: shipment.customerName || (existing && existing.customer_name) || null,
      ...
      console.error("Supabase upsertShipment failed, falling back to in-memory store:", error);
  ...
  if (existingIndex >= 0) {
    ...
    const nextShipment = {
      ...
      currentLat: Object.prototype.hasOwnProperty.call(shipment, "currentLat")
        ? shipment.currentLat
        : (current.currentLat || null),
      currentLng: Object.prototype.hasOwnProperty.call(shipment, "currentLng")
        ? shipment.currentLng
        : (current.currentLng || null),
      originLat: Object.prototype.hasOwnProperty.call(shipment, "originLat")
        ? shipment.originLat
        : (current.originLat || null),
      originLng: Object.prototype.hasOwnProperty.call(shipment, "originLng")
        ? shipment.originLng
        : (current.originLng || null),
      destinationLat: Object.prototype.hasOwnProperty.call(shipment, "destinationLat")
        ? shipment.destinationLat
        : (current.destinationLat || null),
      destinationLng: Object.prototype.hasOwnProperty.call(shipment, "destinationLng")
        ? shipment.destinationLng
        : (current.destinationLng || null),
      customerEmail: shipment.customerEmail || current.customerEmail || null,
      customerName: shipment.customerName || current.customerName || null,
  ...
  const createdShipment = {
    ...
    estimatedDelivery: shipment.estimatedDelivery,
    lastLocation: shipment.lastLocation,
    currentLat: Object.prototype.hasOwnProperty.call(shipment, "currentLat") ? shipment.currentLat : null,
    currentLng: Object.prototype.hasOwnProperty.call(shipment, "currentLng") ? shipment.currentLng : null,
    originLat: Object.prototype.hasOwnProperty.call(shipment, "originLat") ? shipment.originLat : null,
    originLng: Object.prototype.hasOwnProperty.call(shipment, "originLng") ? shipment.originLng : null,
    destinationLat: Object.prototype.hasOwnProperty.call(shipment, "destinationLat") ? shipment.destinationLat : null,
    destinationLng: Object.prototype.hasOwnProperty.call(shipment, "destinationLng") ? shipment.destinationLng : null,
...
async function deleteShipment(trackingNumber) {
  ...
  if (supabase) {
    try {
      ...
      console.error("Supabase deleteShipment failed, falling back to in-memory store:", error);
      // Continue to in-memory fallback below.
    }
  }
...
async function deleteUser(req, email) {
  const { getUsers, setUsers, normalizeEmail } = require("./_auth");
  const users = await getUsers(req);
  const filteredUsers = users.filter(u => normalizeEmail(u.email) !== normalizeEmail(email));
